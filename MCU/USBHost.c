...
extern void mouse_write(uint8_t c);
        ...
        if ( UIF_TRANSFER )                                    
        {
            ...
            if ( r == USB_PID_NAK )
            {
                ...
                case USB_PID_IN:
                    ...
                    if ( r == USB_PID_DATA0 || r == USB_PID_DATA1 ) // ( r == USB_PID_DATA0 && r == USB_PID_DATA1 )
                    {
                    }                                        
                    else if ( r )
                    {
...
{
    static unsigned char SetPort = 0;
        ...
        if ( s == ERR_SUCCESS )
           {
            ...
            {        
                ...
                if (HIDdevice[hiddevice].type == REPORT_USAGE_MOUSE) {
                    mouse_write(RxBuffer[0]); // byte 0: 00000RML  (битовые флаги левой/правой/средней кнопки)
                    mouse_write(RxBuffer[1]); // byte 1: ΔX (signed)
                    mouse_write(RxBuffer[2]); // byte 2: ΔY (signed)
                    mouse_write(RxBuffer[3]); // byte 3: ΔWheel (signed)
                }
            }
        }
        }
...
void parseHIDDeviceReport(unsigned char __xdata *report, unsigned short length, unsigned char CurrentDevive)
{
    ...
    while(i < length)
    {
        ...
        switch(id)
        {
            ...
            case REPORT_USAGE:
                if (!isUsageSet){
                    if (data == REPORT_USAGE_MOUSE) flash_led(); // Подключена мышь.
